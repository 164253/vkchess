...
//m[82]{zu:7,zd:7}
//m[83]nm
$c=$_SESSION['c'];$l=$_SESSION['l'];$eta=S_SESSION['eta'];//$c{$t,kmdlborn,mouseUp,tagUp=0;其他=1;} $l{[4]=0;其他=1;} eta清空
function r(){//只有迴圈能壓的reset
    $i=8;while($i--)$eta[$i]=0;//eta清空
    $i=81;while($i--)$m[$i]&=0xc3ff;$c|=0x3fff0000;//m[$i].$c,zu,zd清空
}
function ml($w,$s){//無規律只能窮舉
    switch($w-$s){
...
function ft($w,$s){//$s=$mou(==ts)
    $wu=$m[$w]>>>($m[$w]&0x3e0?5:0)&0x1f,$su=$m[$s]>>>($m[$s]&0x3e0?5:0)&0x1f;//[$wu]
...
function pf($w,$s){
    $wu=$m[$w]>>>($m[$w]&0x3e0?5:0)&0x1f;//[$wu]
    if($wu^18)return 0;//w沒箭毒蛙
    //w箭毒蛙
        $su=$m[$s]>>>($m[$s]&0x3e0?5:0)&0x1f;//[$su]
...
function mo($w,$s,$epa){//e=evolution p=ispoison a=whichEta//return &1keep &2p &4e
    $mou=$c>>>8&0x7f;
    if(ml($w,$s)){
...
function $sd($d){
    $t=$c>>>31,$s=$c>>>8&0x7f,$i,$j;//$t=$c.$t,$s=mouse
    switch($d){
        case 16:
            for($i=$s-9;$i>=0;$i-=9){
                if($m[$i]>>>14==($t?1:2)){//敵人
        ...
        case 9:case 18:{//這括號不加不能宣告ap,p 編譯器全責
            $ap,p=$m[$s]&0x3e0&&~$c&0x8000;//will p
...
function wh($t){//$t=where只是省字用
    $movelist=[-10,-9,-8,-1,1,8,9,10];//八格遍歷,只能窮舉
    for($th,$tu,$i=0;$i^8;++$i){
        $th=$t+$movelist[$i];
...
function ss(){
    $w=$c>>>8&0x7f,$s=$c&0x7f,$tu=$m[$s]>>>($m[$s]&0x3e0?5:0)&0x1f,$td=$m[$s]>>>($c&0x80?5:0)&0x1f,$t=$c>>>31;
    ...
    if($td==17){//鬣蜥
        ...
        if($m[$w]&0x2000){//吃
            if(pf($w,$s))$m[$w]&=0x3fc0;//wb=$wu=$wd=0
                ...
                if($m[$s]&0x3e0&&~$c&0x80)$m[$w]|=$tu<<5;//if($tu&&!tagUp)$wu=$tu
            }
        }
...
function phpvk($m){
    ...
    $_SESSION['board']=$m;$_SESSION['c']=$c;$_SESSION['l']=$l;$_SESSION['eta']=$eta;
    return implode(",",$m);
}
?>