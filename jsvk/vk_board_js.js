// 2022/5/4
// import {dllexport} from "./jsvk_0.1.js"
// dllexport=window.dllexport
        ...
        if(mp[81]&1){
            if(mp[81]&4){
                if(mp[81]&8^ischange){
                    ...
                    if(0<x&&x<10*bdone&&0<y&&y<bdone)mp=dllexport(10-int(x/bdone));
                }
                else{
                    x=int(x-dist_wd+0.5*bdone);
                    y=int(y-dist_hi-9.5*bdone);
                    if(0<x&&x<10*bdone&&0<y&&y<bdone)mp=dllexport(1+int(x/bdone));
                }
                if(~mp[81]&4)for(let i=1;i^11;++i){$(`#bs${i}`).style.display="none";$(`#bf${i}`).style.display="none";$(`#bgks${i}`).style.display="none";$(`#bgkf${i}`).style.display="none";}
            }
            else{
                x-=dist_wd;
                y-=dist_hi;
            ...
            if(mp[81]&4){
                if(m[81]&8^ischange)for(let i=10;i;--i){$(`#bs${i}`).style.display="block";$(`#bgks${i}`).style.display="block";}
                else for(let i=10;i;--i){$(`#bf${i}`).style.display="block";$(`#bgkf${i}`).style.display="block";}
                for(let i=10;i;--i){}
            }
        }
    }
...
let boardInit=()=>{
    mp=dllexport(9);
    $("#spImg").style=`top:${dist_hi-bdone*1.5}px; left:${dist_wd-bdone*1.5}px; width:${bdone}px;`;
    $("#spImg").src=dir+"vkImage1_2.png";//wait4sql
    $("#spP").style=`top:${dist_hi-bdone*0.5}px; left:${dist_wd-bdone*1.5}px; margin:0px;`;
    $("#fpImg").style=`top:${dist_hi+bdone*9.5}px; left:${dist_wd+bdone*9.5}px; width:${bdone}px;`;
    $("#fpImg").src=dir+"vkImage1_1.png";//wait4sql
    $("#fpP").style=`top:${dist_hi+bdone*10.5}px; right:${dist_wd-bdone*1.8}px; margin:0px;`;//why1.8?idk too.i just try.
    $("#quickc").style=`top:${dist_hi}px; left:${dist_wd}px; width:${bdone*9}px;`;
    $("#quickc").src=dir+"boardline.png";
...
let output=()=>{
    if(mp[81]&2){
        ...
        for(p=0;p^81;++p){$(`#c${p}`).src=dir+`vkImage${mp[p]>>>12&3}_${mp[p]>>>11&1}_${mp[p]>>>10&1}.png`;$(`#c${p}`).style.display="block";}
        let z=mp[82]>>>7&127;
        if(z^127)$(`#c${z}`).src=`vkImagep${(mp[z]>>>13&1)+(mp[z]>>>11&1)}.png`;
        z=mp[82]&127;
        if(z^127)$(`#c${z}`).src=`vkImagep${(mp[z]>>>13&1)+(mp[z]>>>11&1)}.png`;
    }
    else{$("#quickc").style.display="block";for(p=0;p^81;++p)$(`#c${p}`).style.display="none";}
    for(let p=0;p^81;++p){
        if(mp[p]&0x3e0){
            $(`#m${mp[p]>>>15?'':'u'}${p}`).style=`top:${dist_hi+bdone*int(p/9)}px; left:${dist_wd+bdone*(p%9+0.25)}px; width:${bdone/2}px; z-index:3; display:block;`;
            $(`#mu${p}`).src=dir+`vkImage${mp[p]>>>5&0x1f}_${mp[p]>>>14&3}.png`;
            $(`#m${mp[p]>>>15?'u':''}${p}`).style=`top:${dist_hi+bdone*(int(p/9)+0.5)}px; left:${dist_wd+bdone*(p%9+0.25)}px; width:${bdone/2}px; z-index:3;`;
            $(`#mu${p}`).style.display="block";
        }
        else{
            $(`#mu${p}`).style.display="none";
            $(`#m${p}`).style=`top:${dist_hi+bdone*int(p/9)}px; left:${dist_wd+bdone*(p%9)}px; width:${bdone}px; z-index:3;`;
        }
        $(`#m${p}`).src=dir+`vkImage${mp[p]&0x1f}_${mp[p]>>>14&3}.png`;
    }
    let l=mp[81]>>>5&0xff,i;
    for(i=0;i^4;++i)$(`#l${i}`).style.display=l&1<<i?"block":"none";
    for(++i;i^8;++i)$(`#l${i}`).style.display=l&1<<i?"block":"none";
    if(!(mp[81]&1)){
